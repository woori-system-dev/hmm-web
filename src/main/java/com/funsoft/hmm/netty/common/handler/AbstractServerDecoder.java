...
import com.funsoft.hmm.netty.exception.InvalidChecksumException;
...
public abstract class AbstractServerDecoder extends ByteToMessageDecoder {
	...
	protected void decode(ChannelHandlerContext ctx, ByteBuf buffer, List<Object> out) throws Exception {
		...
		logger.debug("{} {}", ctx.channel(), inStr);
		decode(buffer.order(ByteOrder.BIG_ENDIAN), out);
	...
	/**
	 * 메시지 해더를 생성하고, 데이터를 입력한다. ex1) 현재 수신한 모든 버퍼를 삭제한다. ex2) 예외를 발생시켜 채널을 종료한다.
	 * 
	 * @param buffer
	 */
	...
	/**
	 * 버퍼를 이용해 IncomingMessage 생성한다. 예외가 발생할 경우 size만큼 데이터를 읽어서 버린다. Checksum 비교
	 * 
	 * @param buffer
	 */
	private IncomingMessage<?> makeMessageBody(ByteBuf buffer)
			throws InvalidChecksumException, ReflectiveOperationException {
		IncomingMessage<?> incomingMessage;

		try {
			incomingMessage = header.getMessageType().getIncomingClass().getConstructor(header.getClass())
					.newInstance(header);
	...
	/**
	 * Body 처리시 예외가 발생 했을 경우 buffer 처리 ex1) 현재 수신한 모든 버퍼를 삭제한다. ex2) 예외를 발생시켜 채널을
	 * 종료한다. ex3) BodySize 를 버퍼에서 삭제한다. (뒤에 메시지를 보존하기 위해)
	 * 
	 * @param buffer
	 * @param header
	 */
