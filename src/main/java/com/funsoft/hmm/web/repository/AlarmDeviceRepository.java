...
public interface AlarmDeviceRepository extends DefaultRepository<AlarmDevice, CompositePK> {
	...
	@Query(value = "SELECT * FROM ALM_DEVICE WHERE BK_FLCTC_FM = ?1 and DATETIME BETWEEN to_date(?2, 'yyyy-MM-dd hh24:mi:ss') and to_date(?3, 'yyyy-MM-dd hh24:mi:ss')", nativeQuery = true)
